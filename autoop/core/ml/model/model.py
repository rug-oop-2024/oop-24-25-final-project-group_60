...
class Model(Artifact, ABC):
    ...
    def __init__(self, name: str, type: str, **kwargs):
        """Initializes the model with the given name, type, and optional parameters.

        Args:
            name (str): The name of the model.
            type (str): The type of the model (e.g., 'classification' or 'regression').
    ...
    def fit(self, observations: np.ndarray, ground_truths: np.ndarray):
        """Trains the model using the provided observations and ground truth values.

        Args:
            observations (np.ndarray): The input data used for training.
            ground_truths (np.ndarray): The correct labels/values for the training data.
        """
        pass
